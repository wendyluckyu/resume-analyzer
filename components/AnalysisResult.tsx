    ...
    <div className="animate-slide-up space-y-5">
      ...
      <div className="card space-y-4">
        ...
        {result.keywords.matched.length > 0 && (
          <div>
            <p className="mb-2 text-xs text-green-400 font-medium">已匹配 ({result.keywords.matched.length})</p>
            <div className="flex flex-wrap gap-2">
              {result.keywords.matched.map((kw) => (
                <span key={kw} className="tag bg-green-500/10 text-green-400 border border-green-500/20">
                  {kw}
                </span>
              ))}
            </div>
        ...
        {result.keywords.missing.length > 0 && (
          <div>
            <p className="mb-2 text-xs text-red-400 font-medium">缺失 ({result.keywords.missing.length})</p>
            <div className="flex flex-wrap gap-2">
              {result.keywords.missing.map((kw) => (
                <span key={kw} className="tag bg-red-500/10 text-red-400 border border-red-500/20">
                  {kw}
                </span>
              ))}
            </div>
          ...
          {result.suggestions.map((s, i) => (
            ...
            >
              ...
              <p className="text-xs text-slate-400">
                <span className="font-medium text-slate-300">问题：</span>
                {s.issue}
              </p>
              <p className="text-xs text-slate-300">
                <span className="font-medium text-brand-400">建议：</span>
                {s.action}
              </p>
              {s.example && (
                <div className="rounded-lg bg-brand-500/5 border border-brand-500/20 p-3">
                  ...
                  <p className="text-xs text-slate-300 leading-relaxed">{s.example}</p>
                </div>
              )}
            </div>
          ))}
